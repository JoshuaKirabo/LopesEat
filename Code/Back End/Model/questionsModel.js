...
const questionsModel = {
    ...
    getUserGender: async function(userId)
        ...
        getAllergies: async function()
            ...
            checkIfUserHasCompletedQuestions: async function(userId) {
                    //const { userId } = req.params;
                    console.log(userId);
                    
                    try 
                        {
                            const queryResult = await pool.query('SELECT * FROM answers_to_questions WHERE user_id = $1', [userId]);
                            return queryResult.rows.length > 0; // Return true if rows exist, false otherwise
                        } 
                    catch (error) 
                        {
