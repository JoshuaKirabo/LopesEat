...
const loginModel = {
    ...
    loginUser: async function(userId, student) 
        ...
        removeUserFromLoggedIn: async function(userId) // Incase we encounter an error showing the home page on the front end. 
        {
            try 
                {
                    await pool.query('DELETE FROM logged_in_users WHERE user_id = $1', [userId]);
                    console.log(`User ${userId} removed from logged_in_users`);
                } 
            catch (error)
                {
                    console.error('Error removing user from logged_in_users:', error);
