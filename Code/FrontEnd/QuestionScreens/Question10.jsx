...
const Question10 = ({ navigation }) => {
  ...
  const [userGender, setUserGender] = useState(''); // Add userGender state
  const [bodyPainOptions, setBodyPainOptions] = useState([]); 
  ...
  const currentQuestion = 10;
    ...
    useEffect(() => {
      const getGender = async () => {
        ...
        // Directly use if statement to switch options based on gender
        if (gender === 'Male') {
          setBodyPainOptions([
          ...
          setBodyPainOptions([
            { label: 'Sensitive Back', value: 'back', image: require('../Images/women/sensitive-back.png') },
            { label: 'Sensitive Knees', value: 'knees', image: require('../Images/women/sensitive-knees.png') },
            { label: 'Neither', value: 'neither', image: require('../Images/women/no-knee-or-back-pain.png') },
          ]);
        }
      };
      getGender();
