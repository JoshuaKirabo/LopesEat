...
const loginController = {
    handleSignup: async function(req, res) 
        ...
        removeUserFromLoggedIn: async function(req, res) {
            const { userId } = req.body;
        
            try {
                await loginModel.removeUserFromLoggedIn(userId);
                res.json({ success: true, message: 'User removed from logged_in_users' });
            } catch (error) {
                console.error('Error in removeUserFromLoggedIn:', error);
